...
def train_nn(train_df, layers, neurons, r_lambda, alpha, y):
    A = [None] * (layers + 2)
    T = [None] * (layers + 1)
    T[0] = np.array([[0.42, 0.15, 0.4], [0.72, 0.1, 0.54], [0.01, 0.19, 0.42], [0.30000, 0.35000, 0.68000]])
    T[1] = np.array([[0.21000, 0.67000, 0.14000, 0.96000, 0.87000], 
                     [0.87000, 0.42000, 0.20000, 0.32000, 0.89000], 
                     [0.03000, 0.56000, 0.80000, 0.69000, 0.09000]])
    T[2] = np.array([[0.04000, 0.87000, 0.42000, 0.53000], 
...
train_df = np.array([[0.32, 0.68], [0.83, 0.02]])
y = np.array([[0.75, 0.98], [0.75, 0.28]])
train_nn(train_df, 2, [4, 3], 0.25, 0, y)
